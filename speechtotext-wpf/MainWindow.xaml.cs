...
namespace MicrosoftSpeechSDKSamples.WpfSpeechRecognitionSample
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        /// </summary>
        public string CurrentRecognizedText { get; set; }
        ...
        {
            this.baseModelCurrentText.IsEnabled = true;
            this.stopButton.IsEnabled = false;
            if (this.UseBaseModel || this.UseBaseAndCustomModels)
            {
        ...
        private void RecognizingEventHandler(SpeechRecognitionEventArgs e, RecoType rt)
        {
            ...
            //this.GetCurrentText(e.Result.Text);
            this.SetRecognizingText(this.baseModelCurrentText, e.Result.Text);
        ...
        {
            //var log = (rt == RecoType.Base) ? this.baseModelLogText : this.customModelLogText;
            var log = this.baseModelLogText; //this line added
        ...
        /// </summary>
        private void LogRecognitionStart(TextBox log, TextBox currentText)
        {
            string recoSource;
        ...
        private void SetRecognizingText (TextBox textBox, string text)
        {
            this.Dispatcher.Invoke(() =>
            {
                textBox.Text = this.CurrentRecognizedText + text;
                this.RecognizingText = textBox.Text;
            });
        }

        private void SetCurrentText(TextBox textBox, string text)
        {
            //textBox.Text.Remove(textBox.Text.Length);
            this.Dispatcher.Invoke(() =>
            {
                textBox.Text = this.CurrentRecognizedText + text + " ";
                this.CurrentRecognizedText = textBox.Text;
            });
        }
        ...
        private void baseModelCurrentText_TextChanged(object sender, TextChangedEventArgs e)
        {
            if (this.baseModelCurrentText.IsEnabled)
            {
